...
class Client:
    ...
    def index_excel_exercise_completion(self, user_id, exercise_id, is_completed, reason):
        entry = ExcelExerciseCompletion(user_id=user_id, exercise_id=exercise_id, is_completed=is_completed,
                                        reason=reason)
        self.session.add(entry)
        self.session.commit()
        return entry.id
    ...
    def index_question(self, text, choices, difficulty, discrimination, guessing, topics, explanation):
        entry = Question(text=text, choices=choices, difficulty=difficulty, discrimination=discrimination,
                         guessing=guessing, explanation=explanation)
        self.session.add(entry)
        self.session.commit()
        for topic in topics:
            topic_id = self.get_topic_id(topic)
    ...
    def index_demo_user(self, username, email, password):
        entry = auth_user(username=username, email=email, password=password, is_superuser=False, first_name='',
                          last_name='', is_staff=False, is_active=True, date_joined=datetime.now())
        self.session.add(entry)
        self.session.commit()
        return entry.id
