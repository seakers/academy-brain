...
def get_module():
    ...
    builder.add_slide('JD1_1', slide_text[1])
    builder.add_slide('JD1_2', slide_text[2])
    builder.add_slide('JD1_3', slide_text[3])
    builder.add_slide('JD1_4', slide_text[4])
    builder.add_slide('JD1_5', slide_text[5])
    builder.add_slide('JD1_6', slide_text[6])
    builder.add_slide('JD1_7', slide_text[7])
    builder.add_slide('JD1_8', slide_text[8])
    builder.add_slide('JD1_9', slide_text[9])
    builder.add_slide('JD1_10', slide_text[10])
    builder.add_slide('JD1_11', slide_text[11])
    builder.add_slide('JD1_12', slide_text[12])
    builder.add_slide('JD1_13', slide_text[13])
    builder.add_slide('JD1_14', slide_text[14])
    builder.add_slide('JD1_15', slide_text[15])
    builder.add_slide('JD1_16', slide_text[16])
    builder.add_slide('JD1_17', slide_text[17])
    builder.add_slide('JD1_18', slide_text[18])
    builder.add_slide('JD1_19', slide_text[19])
    builder.add_slide('JD1_20', slide_text[20])
    builder.add_slide('JD1_21', slide_text[21])
    builder.add_slide('JD1_22', slide_text[22])
    builder.add_slide('JD1_23', slide_text[23])
    builder.add_slide('JD1_24', slide_text[24])
    builder.add_slide('JD1_25', slide_text[25])
    builder.add_slide('JD1_26', slide_text[26])
    builder.add_slide('JD1_27', slide_text[27])
    builder.add_slide('JD1_28', slide_text[28])
    builder.add_slide('JD1_29', slide_text[29])
...
- The module consists of 3 short lectures:
1. Introduction to Space. Why go to space? Why is space expensive?
2. Overview of a Space Mission. Typical elements and concept of operations for a space mission.
3. Types of Space Missions and Payloads. Functions of space missions, architectures, payload types
""")
slide_text.append("""
...
- Joint Strike Fighter: $\$ 200 B$
- NASA budget: $\$ 20 \mathrm{~B}$
- Airliner: 50-100\$M
- Km of road: $1.5 \$ \mathrm{M}$ world average
""")
slide_text.append("""
...
- Go up at least a few hundreds of $\mathrm{km}$ (overcome gravity, atmospheric drag, etc.)
...
- About $7.5 \mathrm{~km} / \mathrm{s}(\sim 16,700 \mathrm{mph})+1-2 \mathrm{~km} / \mathrm{s}$ more for losses.
...
- The average launch of a satellite on the order of a few hundreds to a few thousands of $\mathrm{kg}$ costs on the order of tens to hundreds of $\$ \mathrm{M}$
- Cost of access to space is about $\$ 5 \mathrm{k}-\$ 20 \mathrm{k} / \mathrm{kg}$
- Higher end for smaller expendable rockets
- Lower end for larger reusable rockets
- SpaceX in particular has disrupted this market and is bringing costs down closer to $\$ 1 \mathrm{k} / \mathrm{kg}$
""")
slide_text.append("""
...
Space mission, space segment, spacecraft bus, payload, orbit launch segment, launch vehicle, ground segment, ground stations, mission control
...
- Frequently used for cost estimation
- More sophisticated cost models are based on complexity indexes
- Combine dozens of complexity factors such as mass, power, data rate, lifetime, flight heritage,etc.
...
- Requires a huge amount of propellant () large rocket ๑) so more efficient to separate this from the spacecraft itself
...
A)Mission Budget
B)Tasking, Scheduling, Control
C)Data Delivery
D)Imaging Concept

The correct choice is A. The ConOps represents the logistics of the mission, not specific design or budget.
""")
slide_text.append("""
...
Detumbling, Deploy antennas and solar panels, perform health tests Transfer to operational orbit
...
Reentry and burn through Earth's atmosphere or move intro "graveyard" orbit
""")
slide_text.append("""
Data Delivery
- Space vs ground processing
- Instrument data received and expensive data processing algorithms are done to filter, calibrate, convert to physical units, and bin the data into images.
- Which of those do we do on-board?
- Centralized vs distributed processing:
- One computer vs. Multiple-dedicated computers?
- Decoupling good for performance,risk, and testing but increases cost
...
Functions of space missions, architectures, payload types
""")
slide_text.append("""
...
- Spectral content: single-band, multispectral, hyper-spectral
...
- RF (L, S, C, X, Ku, K, or Ka bands) optical
...
- Variable accuracy on the order of $\mathrm{cm}$ to meters.
- Payload is a set of L-band antennas
- The most common example is the GPS constellation of $\sim 24$ satellites orbiting Earth at around $\sim 20,000 \mathrm{~km}$ altitude
""")
slide_text.append("""
Example Question
